                    ...
                    <form action="{{URL::to('/dangky-hocbong')}}" method="POST" enctype="multipart/form-data">
                        ...
                        @if(Auth::check())
                            @if(Auth::user()->quyen==2)
                                ...
                                @else
                                    @if($detail->loaihocbong_id==1 && Auth::user()->sinhvien_canbo==1)
                                        @if(!$canRegister)
                                            <button type="submit" name="image[]" disabled class="btn btn-danger text-center text-uppercase" style="margin-top: 20px;">Đã đăng ký</button>
                                        @else
                                            <button type="submit" name="image[]" class="btn btn-danger text-center text-uppercase register_hocbong" style="margin-top: 20px;">Đăng ký</button>
                                        @endif
                                    @elseif($detail->loaihocbong_id==1 && Auth::user()->sinhvien_canbo==0)
                                        <button type="submit" name="image[]" disabled class="btn btn-danger text-center text-uppercase" style="margin-top: 20px;">Không có quyền đăng ký</button>
                                    @else
                                        @if(!$canRegister)
                                            <button type="submit" name="image[]" disabled class="btn btn-danger text-center text-uppercase" style="margin-top: 20px;">Đã đăng ký</button>
                                        @else
                            ...
                            @else
                                @if($detail->hocbong_thoigianketthuc < date('Y-m-d')) 
                                    <button class="btn btn-danger text-center text-uppercase" style="margin-top: 20px;" disabled>Đã hết hạn</button>
                                @else
                                    <button class="btn btn-danger text-center text-uppercase" style="margin-top: 20px;" disabled>Đăng ký</button>
                                @endif
                            @endif
                        @else
                            @if($detail->hocbong_thoigianketthuc < date('Y-m-d')) 
